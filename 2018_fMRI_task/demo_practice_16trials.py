...
from comppsychHelper import * #for jitter
import math
import pyglet
...
ntrigger=0        # number of trigger 5 to start the task
nblk=1        # number of blocks in total
trinumperblk=16 # number of trials in a block

main_dir='C:\\Users\\epulcu\\Desktop\\2018_fMRI\\'
os.chdir(main_dir)
...
filename = 'demo_practice_fMRI_16trial_'+expInfo['Subject Num']
trfilename = 'TRdemo_practice_fMRI_16rial_'+expInfo['Subject Num'] 
dataFile = open(os.path.join(main_dir,'results/demo',filename+'.txt'), 'w')#a simple text file with 'taba-separated-values'
TRFile=open(os.path.join(main_dir,'results/demo',trfilename+'.txt'), 'w')#recording TR onsets
...
Sched=data.importConditions(os.path.join(main_dir,'schedules','2_opt_linkeed_16trial_demo_quickpilot.xlsx'))#2_opt_linkeed_4trial_demo_quickpilot
...
for i in range(Sblk, nblk+1):
    ...
    for thistrial in trialhandle:
        ...
        while countDown.getTime()>0:
            keys = event.waitKeys(maxWait=0.1,keyList=triggerkey)
...
trialhandle.saveAsText(os.path.join(main_dir,'results/demo',filename), dataOut=['Choice_raw', 'Choiceside_raw','RT_raw', 'Winchosen_raw','Losschosen_raw'])
