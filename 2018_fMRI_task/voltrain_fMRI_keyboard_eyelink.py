...
expInfo = {'Subject Num':'', 'nattempt':1,'startBlk':1,'iseyelinked':1}
...
edfname=expInfo['Subject Num'][0:4]+'_fMRI'
...
iseyelinked=int(expInfo['iseyelinked'])
...
if iseyelinked:
    ...
    #edfname = edfname + '.edf'
    el.openDataFile(edfname) # we need to specify this here somewhere/somehow.. added to gui?
...
text = psychopy.visual.TextStim(
        win=win,
        text="Please don't move during the structural scanning\nYou can close your eyes if you like",
        color=[-1, -1, -1]
    )

text.draw()
win.flip()            
psychopy.event.waitKeys(maxWait=3000, keyList=controlkey, modifiers=False, timeStamped=False)

if iseyelinked: #start the recording prior to trial start..?
    el.startRecording(1,1,0,0) 
...
for i in range(Sblk, nblk+1):
    ...
    for thisTrigger_loop in trigger_loop:
        ...
        while continueRoutine:
            ...
            if endExpNow or event.getKeys(keyList=["escape"]):
                if iseyelinked: #shut down the eyetracker recording etc
                    el.stopRecording()
                    el.closeDataFile()
                    el.close()
                core.quit()
    ...
    for thistrial in trialhandle:
        ...
        msg='Fixation trial %d' % (totntrial+1)
        if iseyelinked:
            el.sendMessage(msg) 
        ...
        msg='Choice trial %d' % (totntrial+1)
        if iseyelinked:
            el.sendMessage(msg) 
        #tracker.send_message(msg)
    ...
    # use for timing of stimuli
        ...
        msg='Monitor trial %d' % (totntrial+1)
        if iseyelinked:
            el.sendMessage(msg) 
        #tracker.send_message(msg)
        ...
        if butpress=='none':
            ...
            lossmes.draw()
            win.flip()
        msg='Outcome trial %d' % (totntrial+1)
        if iseyelinked:
            el.sendMessage(msg) 
