...
for ss=1:size(sublist,2)
 %  read data 
    ...
    for i=1:nblk
        ...
        winSurp(i).value=abs(winpech)-mean(abs(winpech));
        winSurp(i).onset=data.outcomesonset_tr((i-1)*tn+1:i*tn);
        winSurp(i).dur=zerocon;
        lossSurp(i).value=abs(losspech)-mean(abs(losspech));
        lossSurp(i).onset=data.outcomesonset_tr((i-1)*tn+1:i*tn);
        lossSurp(i).dur=zerocon;
        
        out_const(i).onset=winSurp(i).onset;
...
mkdir([datadir,'EVfiles/GLM5/',sublist{ss}]);
    for i=1:nblk
        resptmp=[resp(i).onset resp(i).dur resp(i).value];
        save([datadir,'EVfiles/GLM5/',sublist{ss},'/1_response_',realblkname{i},'.txt'],'resptmp','-ascii');

        RTtmp=[RT(i).onset RT(i).dur RT(i).value];
        save([datadir,'EVfiles/GLM5/',sublist{ss},'/2_reactiontime_',realblkname{i},'.txt'],'RTtmp','-ascii');
        
%         ifswitchtmp=[ifswitch(i).onset ifswitch(i).dur ifswitch(i).value];
%         save([datadir,'EVfiles/GLM5/',sublist{ss},'/3_ifswitch_',realblkname{i},'.txt'],'ifswitchtmp','-ascii');

        winevchosentmp=[winEVchosen(i).onset winEVchosen(i).dur winEVchosen(i).value];
        save([datadir,'EVfiles/GLM5/',sublist{ss},'/3_winevchosen_',realblkname{i},'.txt'],'winevchosentmp','-ascii');
        
        lossevchosentmp=[lossEVchosen(i).onset lossEVchosen(i).dur lossEVchosen(i).value];
        save([datadir,'EVfiles/GLM5/',sublist{ss},'/4_lossevchosen_',realblkname{i},'.txt'],'lossevchosentmp','-ascii');
        
        ch_consttmp=[ch_const(i).onset ch_const(i).dur ch_const(i).value];
        save([datadir,'EVfiles/GLM5/',sublist{ss},'/5_ch_const_',realblkname{i},'.txt'],'ch_consttmp','-ascii');
        
        winSurptmp=[winSurp(i).onset winSurp(i).dur winSurp(i).value];
        save([datadir,'EVfiles/GLM5/',sublist{ss},'/6_winsurprise_',realblkname{i},'.txt'],'winSurptmp','-ascii');
        
        lossSurptmp=[lossSurp(i).onset lossSurp(i).dur lossSurp(i).value];
        save([datadir,'EVfiles/GLM5/',sublist{ss},'/7_losssurprise_',realblkname{i},'.txt'],'lossSurptmp','-ascii');

        out_consttmp=[out_const(i).onset out_const(i).dur out_const(i).value];
        save([datadir,'EVfiles/GLM5/',sublist{ss},'/8_out_const_',realblkname{i},'.txt'],'out_consttmp','-ascii');        
    end
%% Make the hrf and plot the corr matrix
names_regressor={'Response','Reaction time','win EV chosen','loss EV chosen','Choice constant','win Surprise','loss Surprise','Outcome constant'};
 for i=1:nblk
     endtime=data.outcomesonset_tr(i*tn,1);
     tmpR=cal_design_matrix(endtime,resp(i),RT(i),winEVchosen(i),lossEVchosen(i),ch_const(i),winSurp(i),lossSurp(i),out_const(i));
    ...
    saveas(f1,[datadir,'EVfiles/GLM5/',sublist{ss},'/designmatrix_',realblkname{i},'.png'])
end
end
...
saveas(f2,[datadir,'EVfiles/GLM5/max_designmatrix_',blkname{i},'.png'])
end