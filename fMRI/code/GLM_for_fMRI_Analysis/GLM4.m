...
mkdir([datadir,'EVfiles/GLM4/',sublist{ss}]);
    for i=1:nblk
        resptmp=[resp(i).onset resp(i).dur resp(i).value];
        save([datadir,'EVfiles/GLM4/',sublist{ss},'/1_response_',realblkname{i},'.txt'],'resptmp','-ascii');

        RTtmp=[RT(i).onset RT(i).dur RT(i).value];
        save([datadir,'EVfiles/GLM4/',sublist{ss},'/2_reactiontime_',realblkname{i},'.txt'],'RTtmp','-ascii');
        
        ifswitchtmp=[ifswitch(i).onset ifswitch(i).dur ifswitch(i).value];
        save([datadir,'EVfiles/GLM4/',sublist{ss},'/3_ifswitch_',realblkname{i},'.txt'],'ifswitchtmp','-ascii');

        evchosentmp=[EVchosen(i).onset EVchosen(i).dur EVchosen(i).value];
        save([datadir,'EVfiles/GLM4/',sublist{ss},'/4_evchosen_',realblkname{i},'.txt'],'evchosentmp','-ascii');
        
        ch_consttmp=[ch_const(i).onset ch_const(i).dur ch_const(i).value];
        save([datadir,'EVfiles/GLM4/',sublist{ss},'/5_ch_const_',realblkname{i},'.txt'],'ch_consttmp','-ascii');
        
        Surptmp=[Surp(i).onset Surp(i).dur Surp(i).value];
        save([datadir,'EVfiles/GLM4/',sublist{ss},'/6_surprise_',realblkname{i},'.txt'],'Surptmp','-ascii');

        out_consttmp=[out_const(i).onset out_const(i).dur out_const(i).value];
        save([datadir,'EVfiles/GLM4/',sublist{ss},'/7_out_const_',realblkname{i},'.txt'],'out_consttmp','-ascii');        
    end
...
names_regressor={'Response','Reaction time','Switch','EV chosen','Choice constant','Surprise','Outcome constant'};
 for i=1:nblk
    ...
    saveas(f1,[datadir,'EVfiles/GLM4/',sublist{ss},'/designmatrix_',realblkname{i},'.png'])
end
end
...
saveas(f2,[datadir,'EVfiles/GLM4/max_designmatrix_',blkname{i},'.png'])
end