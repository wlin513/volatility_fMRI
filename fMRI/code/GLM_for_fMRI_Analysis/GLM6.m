...
for i=1:nblk
    ...
    %regressor 1: stay trials after a positive outcome received
    choice_pos(i).onset=ifswitch(i).onset(winoutlasttrial((i-1)*tn+1:i*tn)==1|lossoutlasttrial((i-1)*tn+1:i*tn)==0);
    choice_pos(i).value=ifswitch(i).value(winoutlasttrial((i-1)*tn+1:i*tn)==1|lossoutlasttrial((i-1)*tn+1:i*tn)==0);
    choice_pos(i).dur=ifswitch(i).dur(winoutlasttrial((i-1)*tn+1:i*tn)==1|lossoutlasttrial((i-1)*tn+1:i*tn)==0);
    %stay trials
    stay_pos(i).onset=choice_pos(i).onset(choice_pos(i).value==-1);
    stay_pos(i).dur=choice_pos(i).dur(choice_pos(i).value==-1);
    stay_pos(i).value=-choice_pos(i).value(choice_pos(i).value==-1);
    %switch trials
...
%% write txt files for each regressor
    mkdir([datadir,'EVfiles/GLM6/',sublist{ss}]);
    for i=1:nblk
        stay_postmp=[stay_pos(i).onset stay_pos(i).dur stay_pos(i).value];
        save([datadir,'EVfiles/GLM6/',sublist{ss},'/1_stay_trials_after_pos_out_',realblkname{i},'.txt'],'stay_postmp','-ascii');
        
        switch_negtmp=[switch_neg(i).onset switch_neg(i).dur switch_neg(i).value];
        save([datadir,'EVfiles/GLM6/',sublist{ss},'/2_switch_trials_after_neg_out_',realblkname{i},'.txt'],'switch_negtmp','-ascii');   
        
        leftouttmp=[leftout(i).onset leftout(i).dur leftout(i).value];
        save([datadir,'EVfiles/GLM6/',sublist{ss},'/3_leftout_trials_',realblkname{i},'.txt'],'leftouttmp','-ascii');     
        
        resptmp=[resp(i).onset resp(i).dur resp(i).value];
        save([datadir,'EVfiles/GLM6/',sublist{ss},'/4_response_',realblkname{i},'.txt'],'resptmp','-ascii');

        RTtmp=[RT(i).onset RT(i).dur RT(i).value];
        save([datadir,'EVfiles/GLM6/',sublist{ss},'/5_reactiontime_',realblkname{i},'.txt'],'RTtmp','-ascii');
         
        reptpostmp=[reptpos(i).onset reptpos(i).dur reptpos(i).value];
        save([datadir,'EVfiles/GLM6/',sublist{ss},'/6_positive_outcomes_',realblkname{i},'.txt'],'reptpostmp','-ascii');
        
        reptnegtmp=[reptneg(i).onset reptneg(i).dur reptneg(i).value];
        save([datadir,'EVfiles/GLM6/',sublist{ss},'/7_negative_outcomes_',realblkname{i},'.txt'],'reptnegtmp','-ascii');
    end
 ...
 for i=1:nblk
    ...
    saveas(f1,[datadir,'EVfiles/GLM6/',sublist{ss},'/designmatrix_',realblkname{i},'.png'])
end
end
...
saveas(f2,[datadir,'EVfiles/GLM6/max_designmatrix_',blkname{i},'.png'])
end