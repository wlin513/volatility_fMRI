...
%sublist={'s01','s02','s03','s04','s05','s06','s07','s08','s09','s10','s11','s12','s13','s14','s15','s16','s17'};
sublist={'s18','s19'};
nblk=4;
tn=80;
...
%% write txt files for each regressor
    ...
    for i=1:nblk
        ...
        save([datadir,'EVfiles/GLM1/',sublist{ss},'/1_response_',realblkname{i},'.txt'],'resptmp','-ascii');

        RTtmp=[RT(i).onset RT(i).dur RT(i).value];
        save([datadir,'EVfiles/GLM1/',sublist{ss},'/2_reactiontime_',realblkname{i},'.txt'],'RTtmp','-ascii');
        
        ifswitchtmp=[ifswitch(i).onset ifswitch(i).dur ifswitch(i).value];
        save([datadir,'EVfiles/GLM1/',sublist{ss},'/3_ifswitch_',realblkname{i},'.txt'],'ifswitchtmp','-ascii');
        
        reptWintmp=[reptWin(i).onset reptWin(i).dur reptWin(i).value];
        save([datadir,'EVfiles/GLM1/',sublist{ss},'/4_winoutcomes_',realblkname{i},'.txt'],'reptWintmp','-ascii');

        reptLosstmp=[reptLoss(i).onset reptLoss(i).dur reptLoss(i).value];
        save([datadir,'EVfiles/GLM1/',sublist{ss},'/5_lossoutcomes_',realblkname{i},'.txt'],'reptLosstmp','-ascii');
    end
end