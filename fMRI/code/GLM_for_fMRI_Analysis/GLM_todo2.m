...
%% @outcome onset
    for i=1:nblk
        ...
        %regressor 5. if win received;(win=1,nowin=0)
        reptwin(i).onset=outonsets;
        reptwin(i).value=winchosen-mean(winchosen);
        reptwin(i).dur=3*constant;   
        %regressor 6. if loss received;(noloss=0,loss=-1)
        reptloss(i).onset=outonsets;
        reptloss(i).value=-losschosen-mean(-losschosen);
        reptloss(i).dur=3*constant;         
...
%         save([datadir,'EVfiles/GLM7/',sublist{ss},'/6_positive_outcomes_',realblkname{i},'.txt'],'reptwintmp','-ascii');
%         
%         reptlosstmp=[reptloss(i).onset reptloss(i).dur reptloss(i).value];
%         save([datadir,'EVfiles/GLM7/',sublist{ss},'/7_negative_outcomes_',realblkname{i},'.txt'],'reptlosstmp','-ascii');
%     end
 
 names_regressor={'choice-ifwin','choice-ifloss','response','choice-mean','out-ifwin','out-ifloss','out-mean'};
 for i=1:nblk
    endtime=data.outcomesonset_tr(i*tn,1);
    tmpR=cal_design_matrix(endtime,choice_ifwin(i),choice_ifloss(i),resp(i),ch_const(i),reptwin(i),reptloss(i),out_const(i));
    R(ss,i,:,:)=tmpR;
