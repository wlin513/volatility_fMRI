...
for ss=1:size(sublist,2)
 %  read data 
    ...
    for i=1:nblk
        ...
        % calculate timing for Eexpected Value for chosen option at choice and absolute Prediction Error(PE) for chosen option at outcome
        ...
        PE(i).value=pech-mean(pech);
        PE(i).onset=data.outcomesonset_tr((i-1)*tn+1:i*tn);
        PE(i).dur=zerocon;
        
        out_const(i).onset=PE(i).onset;
...
mkdir([datadir,'EVfiles/GLM9/',sublist{ss}]);
    for i=1:nblk
        resptmp=[resp(i).onset resp(i).dur resp(i).value];
        save([datadir,'EVfiles/GLM9/',sublist{ss},'/1_response_',realblkname{i},'.txt'],'resptmp','-ascii');

        RTtmp=[RT(i).onset RT(i).dur RT(i).value];
        save([datadir,'EVfiles/GLM9/',sublist{ss},'/2_reactiontime_',realblkname{i},'.txt'],'RTtmp','-ascii');
        
        ifswitchtmp=[ifswitch(i).onset ifswitch(i).dur ifswitch(i).value];
        save([datadir,'EVfiles/GLM9/',sublist{ss},'/3_ifswitch_',realblkname{i},'.txt'],'ifswitchtmp','-ascii');

        evchosentmp=[EVchosen(i).onset EVchosen(i).dur EVchosen(i).value];
        save([datadir,'EVfiles/GLM9/',sublist{ss},'/4_evchosen_',realblkname{i},'.txt'],'evchosentmp','-ascii');
        
        ch_consttmp=[ch_const(i).onset ch_const(i).dur ch_const(i).value];
        save([datadir,'EVfiles/GLM9/',sublist{ss},'/5_ch_const_',realblkname{i},'.txt'],'ch_consttmp','-ascii');
        
        PEtmp=[PE(i).onset PE(i).dur PE(i).value];
        save([datadir,'EVfiles/GLM9/',sublist{ss},'/6_PE_',realblkname{i},'.txt'],'PEtmp','-ascii');

        out_consttmp=[out_const(i).onset out_const(i).dur out_const(i).value];
        save([datadir,'EVfiles/GLM9/',sublist{ss},'/7_out_const_',realblkname{i},'.txt'],'out_consttmp','-ascii');        
    end
%% Make the hrf and plot the corr matrix
names_regressor={'Response','Reaction time','Switch','EV chosen','Choice constant','PE','Outcome constant'};
 for i=1:nblk
     endtime=data.outcomesonset_tr(i*tn,1);
     tmpR=cal_design_matrix(endtime,resp(i),RT(i),ifswitch(i),EVchosen(i),ch_const(i),PE(i),out_const(i));
    ...
    saveas(f1,[datadir,'EVfiles/GLM9/',sublist{ss},'/designmatrix_',realblkname{i},'.png'])
end
end
...
saveas(f2,[datadir,'EVfiles/GLM9/max_designmatrix_',blkname{i},'.png'])
end