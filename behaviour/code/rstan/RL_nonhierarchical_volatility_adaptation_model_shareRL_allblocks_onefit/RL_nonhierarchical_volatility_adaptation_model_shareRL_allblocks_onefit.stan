...
parameters {
  ...
  real alpha_sta_win_tr[nsub]; // learning rate for win stable; one per participant shared between similiar schedules
  real alpha_sta_loss_tr[nsub]; // learning rate for loss stable; one per participant shared between similiar schedules
  real beta_tr[nsub];    // inverse temperature - ; one per participant 
  real win_vol_adpt_tr[nsub]; //difference of alphas for volatile vs. stable for win schedules; one for each participant
  real loss_vol_adpt_tr[nsub]; //difference of alphas for volatile vs. stable for loss schedules; one for each participant
}
...
transformed parameters{
  ...
  for (is in 1:nsub){
    ...
    alpha[1,4,is] = inv_logit(alpha_sta_win_tr[is]);
    alpha[1,3,is] = inv_logit(alpha_sta_win_tr[is]);
    alpha[1,2,is] = inv_logit(alpha_sta_win_tr[is]+win_vol_adpt_tr[is]);
    alpha[1,1,is] = inv_logit(alpha_sta_win_tr[is]+win_vol_adpt_tr[is]);
    
    alpha[2,4,is] = inv_logit(alpha_sta_loss_tr[is]);
    alpha[2,3,is] = inv_logit(alpha_sta_loss_tr[is]+loss_vol_adpt_tr[is]);
    alpha[2,2,is] = inv_logit(alpha_sta_loss_tr[is]);
    alpha[2,1,is] = inv_logit(alpha_sta_loss_tr[is]+loss_vol_adpt_tr[is]);
...
model {
  ...
  for (is in 1:nsub){
   alpha_sta_win_tr[is] ~ normal(0,1);
   alpha_sta_loss_tr[is] ~ normal(0,1);
   win_vol_adpt_tr[is] ~ normal(0,1);
   loss_vol_adpt_tr[is] ~ normal(0,1);
